...
public class SplineComponent : MonoBehaviour
{
    ...
    public Color splineColor = new Color32(74, 119, 255, 255);
    public Color anchorPointColor = new Color32(1, 255, 110, 255);
    public Color anglePointColor = new Color32(85, 253, 122, 255);
    public Color anglePointLinesColor = new Color32(0, 0, 0, 255);
    ...
    void Reset()
    {
        ...
        var differenceAB = (points[0].position - points[1].position) * .3f;

        points[0].handleAPosition = points[0].position + differenceAB;
        points[0].handleBPosition = points[0].position - differenceAB;

        points[1].handleBPosition = points[1].position - differenceAB;
        points[1].handleAPosition = points[1].position + differenceAB;
    }
    ...
    {
        return Mathf.Pow((1 - t), 3) * B + Mathf.Pow((1 - t), 2) * 3 * t * angleA + 3 * (1 - t) * Mathf.Pow((t), 2) * angleB 
            + Mathf.Pow(t, 3) * A;
    }
    ...
    public Vector3 GetTangent(float t)
    {
        ...
        if (closed && i == points.Count - 1) anglePoints = CalculateAnglePoints(P0, P1, P2, P3, t, FindControlPointT(P0), 1);
        var tangent = (anglePoints.R1 - anglePoints.R0).normalized;
