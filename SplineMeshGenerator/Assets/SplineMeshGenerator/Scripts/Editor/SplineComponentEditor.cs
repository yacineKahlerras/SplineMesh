...
public class SplineComponentEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        // flatten X axis button
        if (GUILayout.Button("Flatten X Axis"))
        {
            Undo.RecordObject(target, "Flatten X Axis");
            Flatten(spline.points, "X");
            spline.ResetIndex();
        }
        ...
        if (GUILayout.Button("Flatten Y Axis"))
        {
            ...
            Flatten(spline.points, "Y");
            spline.ResetIndex();
        }

        // flatten Z axis button
        if (GUILayout.Button("Flatten Z Axis"))
        {
            Undo.RecordObject(target, "Flatten Z Axis");
            Flatten(spline.points, "Z");
            spline.ResetIndex();
        }
        GUILayout.EndHorizontal();
    ...
    void Flatten(List<SplineComponent.Anchor> points, string axis)
    {
        Vector3 axisScale = new Vector3(1, 1, 0);
        if (axis == "X") axisScale = new Vector3(0, 1, 1);
        if (axis == "Y") axisScale = new Vector3(1, 0, 1);
        if (axis == "Z") axisScale = new Vector3(1, 1, 0);

        for (int i = 0; i < points.Count; i++)
        {
            points[i].position = Vector3.Scale(points[i].position , axisScale);
            points[i].handleAPosition = Vector3.Scale(points[i].handleAPosition, axisScale);
            points[i].handleBPosition = Vector3.Scale(points[i].handleBPosition, axisScale);
        }
    }
    ...
    void MoveAnchorPoint2D(SplineComponent spline, int i)
    {
        ...
        mousePosition.z = spline.points[i].position.z;
