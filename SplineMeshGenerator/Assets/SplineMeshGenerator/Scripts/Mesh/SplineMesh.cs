...
public class SplineMesh : MonoBehaviour {
    ...
    public float thresholdAngle; // under this threshhold the normals are merged together to get a smooth shape
    float thresholdAngleINIT; // if we changed the threshhold then recalculate normals
    public float normalsGizmosLinesLength; // length of the gizmo 
    public Mesh originalMesh; // the basic flat shape
    public bool meshIsGenerated = false; // equals true when we created a shape and false when we're back to the basic shape

    public List<int> meshLineIndices; // the indices that make the lines of the shape
    public bool drawNormals = false, drawVertexButtons = false, drawGizmoLines = false;
    ...
    public List<float> edgeLoopScales = new List<float>(); // a scale for each edge loop
    List<float> tempEdgeLoopScales = new List<float>(); // for bug fixes
    Dictionary<int, List<int>> temporaryEdgeLoopIndices = new Dictionary<int, List<int>>(); // for bug fixes
    public Dictionary<int, List<int>> mainEdgeLoopIndices = new Dictionary<int, List<int>>(); // gets verts for each edgeLoop
    [HideInInspector] public bool hasUnweldedVerts = false; // gets verts for each edgeLoop

    public Color vertexButtonColor = Color.yellow
        , selectedVertexButtonColor = Color.red
        , gizmosVertexLinesColor = Color.yellow
        , normalsColor = Color.cyan;
    ...
    // gets us back to the original Mesh
    public void BackOriginalMesh()
    {
        mf.mesh = originalMesh;
        meshIsGenerated = false;
    }
    ...
    private OrientedPoint[] GetPath() {
        ...
        mainEdgeLoopIndices = new Dictionary<int, List<int>>();
        temporaryEdgeLoopIndices = new Dictionary<int, List<int>>();
